...
const uploadVideo = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        throw new ApiError(400, `Video upload failed: ${error?.message || error}`);
    }

    try {
        thumbnailFile = await uploadOnCloudinary(thumbnailLocalPath);
    } catch (error) {
        // Video already uploaded — delete it from Cloudinary before bailing
        await cloudinary.uploader.destroy(videoFile.public_id, { resource_type: 'video' }).catch(() => {});
        throw new ApiError(400, `Thumbnail upload failed: ${error?.message || error}`);
    }
